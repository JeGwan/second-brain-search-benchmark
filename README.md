...
> 평가 조건: v3 컨텍스트 기반 채점기, 2회 실행 평균, 답변 모델 Claude Opus 4.8.
> (이번 패스는 5개 원본 문항만 측정 — 변형질문 기반 의미론적 축은 미측정.)

### QMD 주요 분석 피드백 (v3 컨텍스트 기반 재평가)
...
*   **이전(v3 이전) 대비**: 10.5 → 13.0. 채점기가 컨텍스트를 보게 되면서 Q-01·Q-04의 잘못된 환각 감점이 해소됨(각 +1.5, +1.5는 평균 기준). 검색 재현율 지표로 Q-02·Q-03의 책임이 '생성'이 아니라 '검색 청크 커버리지'임을 분리해 확인.
